...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    Returns:
        set: retourne le sous ensemble des mots commençant par une chaine présente 
        dans lstart, contenant une chaine présente dans lmid et finissant par une chaine
          présente dans lstop, avec un nombre de lettres entre nmin et nmax
    ...
    for mot in mots:
        ...
        if not any(mot.startswith(prefix) for prefix in lstart):
            continue

        if not any(mot.endswith(suffix) for suffix in lstop):
            continue

        milieu_valide = False
        corp_du_mot = mot[1:-1]
        for mid in lmid:
            if mid in corp_du_mot:
                milieu_valide = True
                break
